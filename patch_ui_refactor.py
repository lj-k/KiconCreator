"""
patch_ui_refactor.py — 只做 Phase 2 UI 重构
HEAD 已经包含 FA 全量 + FA 网格在 renderLineTabs
要做：把 FA 网格移到样式面板，tab 简化展示
"""
import re

t = open('IconMaker.html', encoding='utf-8').read()

# 验证：FA_MAP 大字典 + renderLineTabs 里的 FA 网格
assert t.count("'fa-") >= 1800, f'FA_MAP 只有 {t.count(chr(39)+"fa-")} 条'
print(f'✅ FA_MAP 已有 {t.count(chr(39)+"fa-")} 条')

# ═══ 2a. renderLineTabs fa 分支：把 FA 网格 UI 简化成只展示当前图标名 ═══
# 找精确锚点：FA 块的开始
fa_start_pat = "} else if(line.mode==='fa'){\n      // 全 FA Free"
start = t.find(fa_start_pat)
assert start > 0, f'fa_start at {start}'
# 找结束锚点：image 分支
end = t.find("    } else { // image", start)
assert end > 0, f'end at {end}'
...
new_img = """    } else { // image —— 仅展示（上传功能移到样式面板的"上传"标签）
...
assert old_img in t, 'old img not found'
...
new_text = """  } else if(l.mode==='text' && _curStyleTab==='fill'){
    // 颜色 tab：单色/渐变切换 + 颜色建议
    add(`<div class="chip-row" style="margin-bottom:4px"><div class="chip ${l.fillMode==='solid'?'active':''}" data-fm="solid">单色</div><div class="chip ${l.fillMode==='gradient'?'active':''}" data-fm="gradient">渐变</div></div>`);
    if(l.fillMode==='solid'){
      add(`<div class="form-row"><div class="lbl">颜色</div><input type="color" id="tc" value="${l.color}"></div>`);
...
t = t.replace(old_text_app_fill, new_text, 1)
print('2d OK: text panels')

# ═══ 2e. fa panel: app → faicon + size ═══
...
new_fa = """  } else if(l.mode==='fa' && _curStyleTab==='faicon'){
    // 图标 tab：颜色 + FA 图标搜索网格
    add(`<div class="form-row"><div class="lbl">颜色</div><input type="color" id="faC" value="${l.faColor||'#fff'}"></div>`);
...
t = t.replace(old_fa_app, new_fa, 1)
print('2e OK: fa panels')

# ═══ 2f. image panel: app → upload + size ═══
...
new_img = """  } else if(l.mode==='image' && _curStyleTab==='upload'){
    // 上传 tab：预览 + 上传 + 清除
...
t = t.replace(old_img_app, new_img, 1)
print('2f OK: image panels')
