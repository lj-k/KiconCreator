...
> 注意，采样密度按曲线跨度取 ~2px 一段（`edgeSteps(span)`，下限 24、上限 240 段）：锯齿的折点与 sin/tan 的峰谷都靠采样点之间的直线连接，间隔太稀会把峰值"削平"（实测 5px 间隔下齿谷被削掉约 8px），因此**不要**为了省算力调稀。
...
- [ ] **填充边界**：过渡宽度 0 时色块之间是硬边（无过渡带）；设 8~20% 后按「过渡样式」出现过渡带（单色 = 两端色中值平涂、渐变 = 带内由前一色渐变到后一色、加深/变浅 = 明显更暗/更亮、透明 = 带内透出背景）；边界形状 sin/tan/锯齿 让分界线按 `A·f(2π·ω·t/S+φ)+k` 起伏且**原点在形状中心**（sin 在 t=0 处不位移、锯齿的齿峰落在中心）；锯齿的 ω 是齿宽、sin/tan 的 ω 是周期数（两套参数行各自独立）；把 ω 拉到最小、A 与 k 拉到最大时不出现 NaN、露底或撕裂；切换边界形状时参数行随之增减（直线 0 行 / sin·tan 4 行 / 锯齿 2 行）且面板重新渲染；过渡带与色块边界严丝合缝（同一条曲线）；撤销、重做、应用预设后边界参数原样恢复；**「包含形状外框」默认不勾选**（此时外框纹丝不动，只有内部色块分界随波形变化），勾选后框线/裁切/填充覆盖一起随波形变形、取消勾选后可精确恢复原外接框，且外框波形首尾接缝无台阶。
