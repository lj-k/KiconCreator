...
├── js/                     # 25 个模块 + data/fa-icons.js，加载顺序 = 依赖顺序（详见第 3 节）
...
│   ├── pack.js             # ④ 导出打包工具（V2.29 新增，零依赖）：ZIP(STORE) 打包器 zipStoreFiles / dataURLBytes / 多尺寸 ICO 组装 buildICOBlob
...
│   ├── exports.js          # ⑭ PNG/JPG/WebP/ICO/Canvas/JSON/HTML/SVG 导出 + 多尺寸/代码/原始图片打包 + 文件名 + 下载历史
...
`index.html` 不再静态书写 `<link>`/`<script>` 标签，而是由页尾**引导器**（内联脚本）按 `steps` 数组顺序逐个动态加载 4 个 CSS、25 个 JS（含 data/fa-icons.js）与 1 个本地预设清单（presets/index.js），并在启动屏上实时显示进度。**顺序即依赖，steps 数组禁止调整**：

```
base/layout/components.css + fa-fonts.css
   └► schema ─► state ─► utils ─► pack ─► images ─► layout ─► builders ─► fa-icons(data) ─► fa ─► panes ─► tabs ─► interactions ─► linkage
                                                                                        │
   main ◄── preview ◄── topbar ◄── theme ◄── fills ◄── imagePane ◄── content ◄── canvas ◄── bgshape ◄── fillLayout ◄── presets/exports/history
```

> **pack.js 的位置（V2.29）**：排在 `utils.js` 之后、`images.js` 之前。它是零依赖纯函数模块
> （不引用任何工程标识符，也只被**运行时**调用），理论上可放在 `main.js` 之前的任意位置；
> 放在 utils 后是为了与"工具层"阅读顺序一致。
...
- `canvasHTMLString()` 生成的独立 HTML 内含 `<script>` 字符串，**必须保留 `<\/script>` 转义**，否则会截断宿主页面。
- 每次导出都会 `pushDownloadHistory`（含 52×52 缩略图 + 全量快照，上限 20 条；`name` 参数记录实际下载文件名）；缩略图由 `renderSnapshotThumb` 按快照数据渲染（需求 2.2）。
- 文件名 `buildFileName`：内容取**所有可见行**拼接——文本用文本、FA 用 FA 代号、图片用原图片名（去扩展名），过滤非法字符并截断（需求 2.35）；时间戳统一用 `timeStamp()`。

**任意尺寸渲染与打包（需求 2.3，V2.29 落地）**：

- `renderIconCanvasAt(size)`：当前状态快照 → `renderSnapshotThumb`（尺寸上限已放宽到 1024）在离屏画布按 size 重画——多尺寸打包与多尺寸 ICO 复用同一渲染管线，保证"zip 里每个尺寸的图 = 预览缩放后的效果"。
- **真实多尺寸 ICO**：`buildMultiSizeICO()` 逐尺寸渲染 PNG（16/32/48/64/128/256），由 pack.js `buildICOBlob` 组装 PNG-in-ICO——**任何情况下 ICO 都输出单个多尺寸 ico 文件**，不受导出尺寸设置影响；V2.28 以前是"单尺寸 PNG 改名 .ico"的冒名实现，已废弃，勿回退。
- **三个打包勾选项**（`multiSizeChk`/`codeChk`/`rawChk`，UI 在 panes.js 下载 pane）语义：任一勾选 → 该次下载输出 **zip**（`buildPackZip(fmt)` 组装，zipStoreFiles 用 STORE 法零依赖打包）：
  | 勾选项 | 作用 |
  |---|---|
  | 多尺寸打包 | 光栅格式（png/jpg/webp）输出 32/64/128/256 各一份（未勾选时只有当前尺寸一份） |
  | 代码打包 | zip 内附带 canvas 代码（.html）+ json 预设（json 走 `presetJSONZipEntry`，含剪裁图片导出方式询问，取消则整次取消） |
  | 原始图片打包 | zip 内附带上传的原始图片（`rawImageZipEntries`：全部行 + 填充色块引用的仓库条目去重，优先 `entry.src` 原始 dataURL，重名自动 `(2)` 递增） |
- 命名：zip 内文件沿用 `KIcon-{尺寸}-{内容}-{时间}.{ext}`，**ico/json/html/canvas/zip 取消{尺寸}**（需求 2.35）；zip 本身 `KIcon-{内容}-{时间}.zip`。下载历史记录实际文件名（zip 时为 zip 名）。
...
| 改导出格式/文件名 | `js/exports.js` | 打包勾选项（多尺寸/代码/原始图片）与多尺寸 ICO 同在本文件；ZIP/ICO 字节组装在 `js/pack.js` |
| 改预设/历史行为 | `js/presets.js` | 下载历史删除按钮 = 右键/长按显示（需求 2.2，V2.29） |
...
23. **打包与 ICO 的语义**（V2.29，需求 2.3/2.35）：任意尺寸渲染必须走 `renderIconCanvasAt`（复用 `renderSnapshotThumb` 快照管线，上限 1024），**禁止**直接缩放主画布位图（会糊）；ICO **始终**输出单个多尺寸文件（16/32/48/64/128/256，PNG-in-ICO），**不要**回退成"单尺寸 PNG 改名 .ico"（Windows 资源管理器不认）；三个打包勾选**任一勾选 → 整次下载为 zip**，zip 内文件命名沿用 `KIcon-{尺寸}-{内容}-{时间}` 而 **ico/json/html/canvas/zip 本身取消{尺寸}**；`canvasHTMLString` 的 `<\/script>` 转义与 `zipStoreFiles` 的 UTF-8 标志位（0x0800）都不可去掉。
...
- [ ] **导出打包（V2.29）**：不勾选打包时 PNG/JPG/WEBP 输出单文件（带{尺寸}）；勾选任一打包项后同一按钮输出 zip（多尺寸勾选 → 32/64/128/256 四份、代码打包 → zip 内有 canvas.html + json、原始图片打包 → zip 内有上传的原图）；ICO 按钮输出**真实多尺寸 .ico**（文件名不含{尺寸}，Windows 资源管理器/浏览器 favicon 均可识别）；下载历史条目名与实际下载文件名一致（zip 时为 zip 名）。
