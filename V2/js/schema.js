/* ============================================================
   ...
   版本：V0.10（V2.27：FILL_PARAM_DEFS 新增 edge.wave——波浪形状的 ω 半圆宽度（1~100%，默认 20=2×A → 恰为半圆））
        V0.09（V2.25：FILL_PARAM_DEFS 新增 edge.frame——边界形状是否包含形状外框，默认 false）
...
const FILL_PARAM_DEFS = {
  ...
  /* 填充边界（需求 2.3 标签"填充边界"）：过渡带宽 + 边界形状 + 形状函数的边界参数
     范围为"函数安全范围"（需求 2.3：防止除零/无穷大，渲染再做 10× 画布截断）：
       A 振幅/高度 0~50%（画布边长的百分比）
       ω sin/tan = 横跨画布的完整周期数（0.1~10，步进 0.1）；
         锯齿 = 齿宽（1~100%）；波浪 = 半圆宽度（1~100%）
       φ 相位/偏移 −180~180°；k 垂直偏移 −50~50%（波浪按需求只有 A/ω/φ，无 k） */
  ...
  'edge.wave':     { label: 'ω',        min: 1,   max: 100, def: 20,   unit: '%' },  // 波浪专用：半圆宽度（默认 2×A=20 → 恰为半圆）
